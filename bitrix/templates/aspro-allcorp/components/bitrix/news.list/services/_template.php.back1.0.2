...
<? if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) die(); ?>
<div
    class="item-views list <?= ($arParams["IMAGE_POSITION"] ? "image_" . $arParams["IMAGE_POSITION"] : "") ?> <?= ($templateName = $component->{"__parent"}->{"__template"}->{"__name"}) ?>">
    <? // top pagination?>
    ...
    <div class="<?= ($arParams["SHOW_TABS"] == "Y" ? "tab-content" : "group-content") ?>">
        ...
        <div class="row">
            ...
            <?
                ...
                $bActiveDate = strlen(
                    ...
                    ) || ($arItem["DISPLAY_ACTIVE_FROM"] && in_array(
                            "DATE_ACTIVE_FROM",
                            $arParams["FIELD_CODE"]
                        ));
            ?>
            ...
            <? if ($bActiveDate): ?>
                <div class="period">
                    <? if (strlen($arItem["DISPLAY_PROPERTIES"]["PERIOD"]["VALUE"])): ?>
                        <span
                            class="label label-info"><?= $arItem["DISPLAY_PROPERTIES"]["PERIOD"]["VALUE"] ?></span>
                    <? else: ?>
            ...
            <? endif; ?>
            <? if ($bDetailLink): ?>
                <a href="<?= $arItem["DETAIL_PAGE_URL"] ?>" class="btn btn-primary btn-sm"><?= GetMessage(
                        "TO_ALL"
                        ...
                        <a href="<?= $arItem["FIELDS"]["DETAIL_PICTURE"]["SRC"] ?>" class="img-inside fancybox">
                            ...
                            <? if ($arItem["FIELDS"]["PREVIEW_PICTURE"]): ?>
                                <img src="<?= $arItem["FIELDS"]["PREVIEW_PICTURE"]["SRC"] ?>"
                                     alt="<?= $arItem["FIELDS"]["PREVIEW_PICTURE"]["ALT"] ?>"
                                     title="<?= $arItem["FIELDS"]["PREVIEW_PICTURE"]["TITLE"] ?>"
                                     class="img-responsive"/>
                            <? else: ?>
                                <img src="<?= SITE_TEMPLATE_PATH ?>/images/noimage.png" alt="<?= $arItem["NAME"] ?>"
                                     title="<?= $arItem["NAME"] ?>" class="img-responsive"/>
                            <? endif; ?>
            ...
            <div id="<?= $this->GetEditAreaId($arItem["ID"]) ?>"
                ...
                <div class="col-lg-6 col-md-12 col-xs-12">
                    ...
                    <? elseif ($arParams["IMAGE_POSITION"] == "right"): ?>
                        ...
                        <div class="col-lg-5 col-md-12"><?= $imagePart ?></div>
                    <? else: ?>
                        <div class="col-lg-5 col-md-12"><?= $imagePart ?></div>
                        <div class="col-lg-7 -col-md-12">
                            <div class="text"><?= $textPart ?></div>
        ...
        <? if (is_array($arResult["SECTION"]["PATH"])): ?>
            ...
            <? if (strlen($arCurSectionPath["DESCRIPTION"]) && strpos(
                    $_SERVER["REQUEST_URI"],
                    "PAGEN"
                ) === false
            ): ?>
